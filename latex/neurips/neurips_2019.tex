...
Java, one of the most prolific programming languages on GitHub, is a statically typed language with a high volume of API documentation. With has a variety of tools for parsing source code~\citep{parr2013definitive, hosseini2013javaparser, kovalenko2019pathminer} and natural language~\citep{manning2014stanford, grella2018non}, it makes an ideal language for both implementation and analysis. Our dataset consists of Java repositories on GitHub, and their accompanying developer documents. All projects in our dataset have a collection of source code files and multiple related repositories on GitHub.
...
Our goal is to connect these two graphs using a common representation for source code and natural language. Absent any explicit \texttt{@link} or \texttt{@see} annotations, in order to relate these two graphs, we must infer the shared semantic entities.
...
\begin{itemize}
    ...
    \item Single synthetic graph containing code and documentation entities related by lexical matching (LEX)
\end{itemize}

For each model, we evaluate the trained model on link prediction in the same setting and every other setting.
